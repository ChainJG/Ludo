...
Arena offers v1–v4, Compact search v6, experimental Neural v7, ChatGPT Tactician v8 and Fable v9. Search v5 remains available in Warzone and the CLI: its deeper search exceeded the original 50 ms browser budget. Arena now allows bots 2,000 ms per decision because browser speed varies widely by device; every bot still uses a deterministic node budget, so a larger time limit only avoids false disqualifications. Arena defaults to Fable v9, which passed the held-out gate against Heuristic v2, with a 1,500 ms delay between bot moves. Compact search v6 limits expansion to 72 nodes; it is an experimental alternative, not a promoted champion.
